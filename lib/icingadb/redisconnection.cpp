...
RedisConnection::RedisConnection(boost::asio::io_context& io, String host, int port, String path, String password,
	...
	: m_Host(std::move(host)), m_Path(std::move(path)), m_Port(port), m_Password(std::move(password)),
	  ...
	  m_Strand(io), m_Started(false), m_QueuedWrites(io), m_QueuedReads(io), m_LogStatsTimer(io), m_Parent(parent)
{
